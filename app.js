...
function avatarHtml(u,big=false){let letter=(u?.name||'Б')[0];return `<div class="avatar ${big?'big':''}">${u?.avatar?`<img src="${escapeAttr(u.avatar)}" onerror="this.parentNode.textContent='${letter}'">`:letter}</div>`}
function escapeAttr(s){return String(s||'').replace(/"/g,'&quot;')}
function checks(n){return $all(`input[name=${n}]:checked`).map(x=>x.value)}
function multi(name,arr=[],limit=8){return S.wrestlers.map(w=>`<label class="choice"><input type="checkbox" name="${name}" value="${w.id}" ${(arr||[]).includes(w.id)?'checked':''} onchange="BTP.limitChecks('${name}',${limit})">${w.name} <span class="muted small">${w.rank}</span></label>`).join('')}
...
function board(){let province=localStorage.getItem(DB+'_filter')||'';let rows=[...S.users].filter(u=>!province||u.province===province).sort((a,b)=>(b.score||0)-(a.score||0)).slice(0,100).map((u,i)=>`<tr><td>${i+1}</td><td>${u.name}<div class="muted small">${u.province||'-'} · Lv ${u.level}</div></td><td>${u.xp||0}</td><td><b>${u.score||0}</b></td></tr>`).join('');return `<h2>Чансааны самбар</h2><div class="card"><label>Аймгаар шүүх</label><select onchange="BTP.filterProvince(this.value)"><option value="">Бүгд</option>${provinces.map(p=>`<option ${province===p?'selected':''}>${p}</option>`).join('')}</select><br><br><table class="table"><tr><th>#</th><th>Нэр</th><th>XP</th><th>Оноо</th></tr>${rows||'<tr><td colspan="4">Одоогоор хэрэглэгч алга.</td></tr>'}</table></div>`}
function filterProvince(v){localStorage.setItem(DB+'_filter',v);render()}
function profile(){let u=me();if(!u)return authBox();let pct=u.totalPredictions?Math.round((u.correctPredictions/u.totalPredictions)*100):0;return `<h2>Профайл</h2><div class="grid"><div class="card span4">${avatarHtml(u,true)}<h2>${u.name}</h2><p class="muted">${u.phone}</p><div class="score">${u.xp} XP</div><p>Level ${u.level}</p><div class="progress"><i style="width:${Math.min(100,(u.xp%150)/150*100)}%"></i></div><p class="muted small">Таасан хувь: ${pct}%</p>${(u.medals||[]).map(m=>`<span class="medal">🎖 ${m}</span>`).join('')||'<span class="medal">Шинэ гишүүн</span>'}</div><div class="card span8"><h2>Профайл засах</h2><label>Зураг URL</label><input id="pAvatar" value="${escapeAttr(u.avatar||'')}" placeholder="https://..."><br><br><label>Нэр</label><input id="pName" value="${escapeAttr(u.name||'')}"><br><br><label>Аймаг</label><select id="pProvince">${provinceOptions(u.province)}</select><br><br><label>Нас</label><input id="pAge" type="number" value="${escapeAttr(u.age||'')}"><br><br><button onclick="BTP.saveProfile()">Хадгалах</button></div></div>`}
